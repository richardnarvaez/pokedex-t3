        ...
        <div className="container mx-auto">
          ...
          <div className="mb-8 flex flex-col gap-4 md:flex-row md:justify-between">
            <input
              ...
              className="h-10 rounded-md border border-gray-200 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-950 focus:ring-offset-2"
              value={search}
              onChange={(e) => {
          ...
          <div className="flex justify-center p-4 h-12">
            {isFetching && (
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-3 lg:grid-cols-4">
              {data?.pages.flatMap((page) => page.items).map((pokemon) => (
                <>{pokemon?.id && <PokemonCard key={pokemon.id} pokemon={pokemon} />}</>
              ))}
